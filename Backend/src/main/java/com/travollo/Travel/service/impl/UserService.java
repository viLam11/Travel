...
public class UserService implements UserInterface {
    ...
    public ResponseEntity<Object> verifyUser(VerifyDTO verifyDTO) {
        User user = theUserRepo.findByEmail(verifyDTO.getEmail()).orElseThrow(() -> new CustomException(HttpStatus.NOT_FOUND, "User not found"));
        // Check if verification code has expired
        if (user.getExpiredAt().isBefore(LocalDateTime.now())) {
            return ResponseEntity.badRequest().body(new ErrorResponse(400, "Verification code has expired. Please request a new one"));
        }
        // Check if verification code matches
        if (!verifyDTO.getVerificationCode().equals(user.getVerificationCode())) {
            return ResponseEntity.badRequest().body(new ErrorResponse(400, "Invalid verification code"));
        }
        // Verify user
