...
import { MapPin, Calendar, Clock, Users, Heart, Share2, ChevronLeft, ChevronRight, Info, Lock } from 'lucide-react';
...
import { loadServiceDetail, clearServiceDetail } from '../../../store/slices/serviceDetailSlice';
...
const ServiceDetailPage: React.FC = () => {
  ...
  // Redux state
  const { data: service, loading, error } = useSelector(
  ...
  const [roomPaymentMethod, setRoomPaymentMethod] = useState<'card' | 'online'>('card');
  ...
  const getDaysInMonth = (monthKey: string) => {
    ...
    const days: (null | { day: number; price: string | null; available: boolean })[] = [];
  ...
  // Calculate totals
  const totalPrice = (adultCount * service.priceAdult) + (childCount * service.priceChild) + 
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-8">
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-3">
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 flex-1">{service.name}</h1>
          </div>
          <div className="flex items-center justify-between">
            <div className="flex flex-wrap items-center gap-3 sm:gap-4 text-sm text-gray-600">
              ...
              <span className="text-orange-500 font-medium">30K+ lượt khách</span>
            </div>
            <div className="flex items-center gap-3 ">
              ...
              >
                <Heart className={`w-4 h-4 ${isFavorite ? 'fill-orange-500 text-orange-500' : ''}`} />
                Wishlist
              </button>
          ...
          <div className="lg:col-span-2 space-y-6">
              ...
              <div className="flex gap-6 sm:gap-8">
                <button
                  ...
                  className={`pb-3 font-semibold transition-colors relative ${
                    activeTab === 'info' ? 'text-gray-900' : 'text-gray-500 hover:text-gray-700'
                  }`}
                >
                  Thông tin
                  {activeTab === 'info' && (
            ...
            {activeTab === 'info' && (
              <div className="space-y-6">
                <div>
                  <h3 className="text-xl font-bold text-gray-900 mb-3">Thông tin</h3>
                  <p className="text-gray-700 leading-relaxed text-sm sm:text-base">{service.description}</p>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4">
                  {service.features.map((feature, idx) => (
                    <div key={idx} className="flex flex-col items-center text-center p-4 bg-gray-50 rounded-xl hover:bg-gray-100 transition-colors">
                ...
                <div>
                  <h3 className="text-xl font-bold text-gray-900 mb-4">Đặt lịch</h3>
                        ...
                        {getDaysInMonth(selectedMonth).map((day, idx) => (
                          <div key={idx} className="aspect-square">
                            {day ? (
                              ...
                              >
                                <span>{day.day}</span>
                                {day.price && <span className="text-[10px] sm:text-xs font-semibold">{day.price}</span>}
                              </button>
                            ) : (
                ...
                <div>
                  <h3 className="text-xl font-bold text-gray-900 mb-4">Vị trí maps</h3>
        ...
        >
          ...
          >
            ...
            <p className="text-gray-600 text-center mb-8 leading-relaxed">
              Bạn cần đăng nhập để thực hiện đặt phòng và trải nghiệm đầy đủ các tính năng của chúng tôi.
            </p>
