// src/components/page/hotelFilter/HotelFilterSidebar.tsx
import React, { useState } from 'react';
import { X, ChevronUp, ChevronDown, Star, TrendingUp, Hotel, MapPin, Wifi, Waves, Dumbbell, Sparkles, Car, UtensilsCrossed } from 'lucide-react';
...
const SORT_OPTIONS = [
    { value: 'popular', label: 'Phổ biến nhất', description: 'Sắp xếp theo lượt xem và booking' },
    { value: 'price-low', label: 'Giá thấp đến cao', description: 'Từ rẻ đến đắt' },
    { value: 'price-high', label: 'Giá cao đến thấp', description: 'Từ đắt đến rẻ' },
    { value: 'rating', label: 'Đánh giá cao nhất', description: 'Từ 5 sao đến 1 sao' },
];
...
const HotelFilterSidebar: React.FC<HotelFilterSidebarProps> = ({
    ...
    const [isStarsExpanded, setIsStarsExpanded] = useState(true);
    ...
    const sidebarContent = (
        <div className="space-y-6">
            ...
            <div className="border-b border-gray-200 pb-6">
                <button
                    onClick={() => setIsStarsExpanded(!isStarsExpanded)}
                    className="flex items-center justify-between w-full mb-4"
                >
                    <span className="font-semibold text-gray-900 flex items-center gap-2">
                ...
                {isStarsExpanded && (
                    <div className="space-y-2">
                        {[5, 4, 3].map((stars) => (
                            <label
                                key={stars}
                                className={`flex items-center p-3 rounded-lg cursor-pointer transition-all ${selectedStars.includes(stars)
                                    ? 'bg-orange-50 border-2 border-orange-500'
                                    : 'bg-gray-50 border-2 border-transparent hover:border-gray-300'
                                    } `}
                            >
                                ...
                                <div className="ml-3 flex items-center gap-1">
                                    {Array.from({ length: 5 }).map((_, i) => (
                                        <Star
                                            key={i}
                                            className={`w-4 h-4 ${i < stars ? 'text-orange-400 fill-orange-400' : 'text-gray-300'
                                                } `}
                                        />
                                    ))}
                                </div>
                ...
                {isAmenitiesExpanded && (
                    <div className="space-y-2">
                        {AMENITIES_OPTIONS.map((amenity) => (
                            <label
                                key={amenity.id}
                                className={`flex items-center p-3 rounded-lg cursor-pointer transition-all ${selectedAmenities.includes(amenity.id)
                                    ? 'bg-orange-50 border-2 border-orange-500'
                                    : 'bg-gray-50 border-2 border-transparent hover:border-gray-300'
                                    } `}
                            >
